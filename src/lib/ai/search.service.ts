...
export async function fetchClimateData(lat: number, lon: number): Promise<string> {
  try {
    ...
    url.searchParams.set('daily', 'temperature_2m_max,temperature_2m_min,precipitation_sum,relative_humidity_2m_max')
    url.searchParams.set('start_date', '2018-01-01')
    url.searchParams.set('end_date', '2022-12-31')
    ...
    const monthly: Record<number, { maxTemps: number[]; minTemps: number[]; humidity: number[] }> = {}
    for (let m = 1; m <= 12; m++) monthly[m] = { maxTemps: [], minTemps: [], humidity: [] }

    data.daily.time.forEach((date: string, i: number) => {
      const month = parseInt(date.split('-')[1])
      if (data.daily.temperature_2m_max[i]) monthly[month].maxTemps.push(data.daily.temperature_2m_max[i])
      if (data.daily.temperature_2m_min[i]) monthly[month].minTemps.push(data.daily.temperature_2m_min[i])
      if (data.daily.relative_humidity_2m_max[i]) monthly[month].humidity.push(data.daily.relative_humidity_2m_max[i])
    })
