...
const PROVIDER_CONFIG: Record<AIProvider, { baseURL: string; apiKeyEnv: string; defaultModel: string }> = {
  openrouter: {
...
const TASK_MODEL_OVERRIDES: Partial<Record<AITask, string>> = {
  clarification_check:  'openai/gpt-4o-mini',   // fast + cheap for gap detection
  followup_questions:   'openai/gpt-4o-mini',
  climate_analysis:     'openai/gpt-4o-mini',    // mostly template + data injection
  report_executive_summary:  'anthropic/claude-sonnet-4-5',
  report_market_analysis:    'anthropic/claude-sonnet-4-5',
  report_business_model:     'anthropic/claude-sonnet-4-5',
  report_financial_projection: 'anthropic/claude-sonnet-4-5',
  report_risk_mitigation:    'anthropic/claude-sonnet-4-5',
  report_conclusion:         'anthropic/claude-sonnet-4-5',
  financial_projection:      'anthropic/claude-sonnet-4-5',
  market_research:           'anthropic/claude-sonnet-4-5',
}
...
export async function callAIJSON<T = unknown>(request: AIRequest): Promise<T> {
  const response = await callAI({ ...request, maxTokens: request.maxTokens || 2000 })
