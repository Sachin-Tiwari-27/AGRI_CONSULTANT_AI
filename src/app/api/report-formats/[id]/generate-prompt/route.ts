...
2. The prompt must instruct the AI to write approximately {{word_count_target}} words (within 20%)
...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  // Build the meta-prompt by substituting the section's own metadata
  const metaPrompt = PROMPT_GENERATOR_TEMPLATE.replaceAll(
    "{{section_title}}",
    section.title,
  )
  ...
  try {
    // Call the gateway directly — no callAI() wrapper needed here since we
    // are supplying the full prompt ourselves (not going through prompts.store).
